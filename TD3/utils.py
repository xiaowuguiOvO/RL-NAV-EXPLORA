...
def get_cell_position_from_coords(coords, map_info, check_negative=True):
    '''
    from world coordinates to cell position in the map
    :param coords: world coordinates, shape (N, 2)
    :param map_info: MapInfo object
    :param check_negative: whether to check if the cell position is negative
    :return: cell position in the map, shape (N, 2)
    '''
    single_cell = False
...
def get_updating_node_coords(location, updating_map_info, check_connectivity=True):
    ...
    if not check_connectivity:
        ...
        for i, cell in enumerate(nodes_cells):
            # print(cell, updating_map_info.map.shape)
        ...
        nodes_cells = get_cell_position_from_coords(nodes, updating_map_info).reshape(-1, 2)
        for i, cell in enumerate(nodes_cells):
            # print(cell, free_connected_map.shape)
